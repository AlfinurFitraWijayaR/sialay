...
import { PLAYING_POSITIONS } from '../../lib/player-utils'
...
function CreatePlayerPage() {
  ...
  // Administration documents state
  const [formReg, setFormReg] = useState<'ada' | 'belum_ada'>('belum_ada')
  const [formKk, setFormKk] = useState<'ada' | 'belum_ada'>('belum_ada')
  const [formAkte, setFormAkte] = useState<'ada' | 'belum_ada'>('belum_ada')
  const [formPhoto, setFormPhoto] = useState<'ada' | 'belum_ada'>('belum_ada')
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white rounded-lg border border-[#e2e8f0] p-6 shadow-sm space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="space-y-1.5 flex-1">
              <div className="flex items-center gap-2">
                ...
                >
                  {photoPreview ? 'Ganti Foto Siswa' : 'Pilih Foto Siswa'}
                </button>
                {photoPreview && (
                  <button
                    type="button"
                    onClick={handleClearPhoto}
                    className="px-2.5 py-1.5 text-xs font-medium text-[#972828] hover:bg-[#FEF2F2] rounded-lg border border-[#FECACA] transition cursor-pointer"
                  >
                    Hapus Foto Siswa
                  </button>
                )}
          ...
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#0f172a] border-b border-[#e2e8f0] py-2">
            2. Posisi Bermain & Status Keanggotaan
          </h2>
          ...
          {/* Data Administrasi */}
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#0f172a]  border-b border-[#e2e8f0] py-2">
            3. Berkas Administrasi Siswa
          </h2>

          <div className="space-y-2.5 grid grid-cols-1 md:grid-cols-2 gap-2">
